...
export default function RobotCard({ robot, sharedProps }) {
  ...
  console.log("👀 Rendering image for", robot.robot_id, imageBase64?.slice(0, 20));
...
useEffect(() => {
  const imageEvent = `robot_image_${robot_id}`;

  const handleImage = (imageData) => {
    setImageBase64(imageData.image_base64);
  };

  socket.on(imageEvent, handleImage);

  return () => {
    socket.off(imageEvent, handleImage);
  };
}, [robot_id]);
...
<div style={{ fontSize: '20px', marginTop: '12px', marginBottom: '4px' }}>
  ...
  <div style={{
    ...
    borderRadius: '5px',
    overflow: 'hidden',
    marginTop: '4px'  // slightly more breathing room above bar
  }}>
<div style={{
  width: `${battery}%`,
  height: '100%',
...
<Slider
  ...
  onChange={(opt) => sendCommand(robot_id, opt.toLowerCase())}
  disabled={isInteractionBlocked || robotStates[robot_id]?.activation == 0 || connection !== 'connected'}
