...
export default function renderRobotCard(robot, sharedProps) {
  ...
  return (
    <div key={robot_id} style={{ border: '2px solid #444', borderRadius: '10px', padding: '20px', margin: '20px auto', maxWidth: '450px', background: '#f4f4f4', opacity: loading ? 0.5 : 1 }}>
      ...
      <Slider label="Operation Mode" options={['Mapping', 'Navigation']} onChange={(opt) => sendCommand(robot_id, opt.toLowerCase())} disabled={loading} />
      <Slider label="Homing" options={['Undock', 'Dock']} onChange={(opt) => sendCommand(robot_id, opt.toLowerCase())} disabled={loading} />
